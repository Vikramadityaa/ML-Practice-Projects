...
from db_helper import get_db_session, fetch_questions, save_answer, fetch_scoreboard, register_user, authenticate_user, save_question
...
def get_questions(topic):
    ...
    with get_db_session() as db:
        ...
        return [{"id": q.id, "topic": q.topic, "question_text": q.question_text, "date_added": str(q.date_added)} for q in questions]



@app.post("/submit_answer")
def submit_answer(user_id: int, question_id: int, answer_text: str):
    with get_db_session() as db:
        ...
        save_answer(user_id, question_id, answer_text, evaluation["score"])
